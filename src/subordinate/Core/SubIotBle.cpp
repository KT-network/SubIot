...
void SubIotBle::setWaitState(bool state) {
    this->wait = state;
}

bool SubIotBle::getWaitState() {
    return this->wait;
}
...
void SubIotBle::begin(WorkState state) {
    ...
    this.bleName = SUB_IOT_FACTORY_BLE_NAME;
#else
    ...
    if (state == WORK_STATE_FACTORY_CONFIG) {
        ...
        Serial.println("[Ble] System Factory Start System Config ");
